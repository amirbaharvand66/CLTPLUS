function tau_rt_on_hole(epsilon, theta, R, alpha, beta, s_k, h_k, c_k, mkr)
% Calculate hoop stress distribution on hole edge
...
fig_sigma_r = figure (103);
hold on
set(fig_sigma_r, 'Pos', [0 0 800 600])
plot(deg2rad(theta), sigma_p(:, 3), 'LineStyle', mkr, 'Color', 'k', 'LineWidth', 2, 'DisplayName', sprintf('%d%sHole radius', R, '$\times$'))
set(gca, 'FontSize', 15)
xticks([0 + epsilon, pi/2, pi, 3*pi/2, 2*pi - epsilon])
xticklabels({'0', '$\pi/2$', '$\pi$', '$3\pi/2$', '$2\pi$'})
xlabel('$\theta$[rad]','Interpreter','latex', 'FontSize', 25)
ylabel('$\tau_{r\theta}/p$' ,'Interpreter','latex', 'FontSize', 25)
legend('Interpreter','latex', 'FontSize', 20, 'Location', 'southeast')
